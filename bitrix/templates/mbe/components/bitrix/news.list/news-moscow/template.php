...
$total = CIBlockElement::GetList(Array(), array('IBLOCK_ID'=>5, 'PROPERTY_CENTER' => $arParams['CENTERS'], 'ACTIVE' => 'Y'), array(), false, array('ID', 'NAME'));
?>


<div id="news-msk" class="news-list page-tab-content active" style="min-height: 200px;">
  ...
  {?>
    <div class="cols">
      ...
      <? $GLOBALS['promo_filter'] = array('PROPERTY_CENTER' => $arParams['CENTERS']); ?>
    ...
    if (count($arResult['ITEMS']) > 2)
    {
      ...
      {?>
        <a href="/bitrix/templates/mbe/ajax/news_more.php?city=<?= $city_id;?>&n=<?= $nPageSize;?>&iNumPage=<?= $_GET['iNumPage'];?>&total=<?= $total;?>" data-container="#news-msk" class="custom-link custom-link--more custom-link--right more-link">
