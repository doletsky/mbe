	...
	var MBE = (function() {
		...
		return {
			...
			initCenters: function() {
				...
				$sel.body.on("click", ".show-city", function(e) {
					var $item = $(this),
				...
				function hoverArea(areaCode) {
					...
					if($cities.length) {
						...
						$cities.each(function() {
							...
							$city.css({
								"z-index": "10000",
								"transform": "translateX(" + (rmapObjectBoxWidthAdd / 2) * (cityLeft < 0.5 ? -1 : 1) + "px) translateY(" + (rmapObjectBoxHeightAdd / 2) * (cityTop < 0.5 ? -1 : 1) + "px)"
							});
